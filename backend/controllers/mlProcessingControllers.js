...
exports.medicalChatBot = async (req, res) => {
  ...
  const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
    var result;
    async function complete(prompt) {
      const completion = await model.generateContent(["You are a friendly medical chatbot here to help patients with their medical queries. Remember to suggest doctor's consultation for severe cases first before suggesting something. But be sure to give some suggestions before suggesting to doctor. Also make the suggestions a bit detailed." + prompt])
        .then((res) => {
        ...
        .catch((err) => {
          ...
          return res.status(500).json({
            message:"Gemini server Busy"
          })
        });
    }
    const message = req.body.input;
    console.log(message);
    const prompt = `Give your answer in 3 to 4 lines, do not use highlighting or other stuff for the query which is delimited with triple backticks \n
    user query: '''${message}'''`;
